...
public class FileUtil {
    ...
    public static File getCacheDir(String dirName) {
        File result;
        if (existsSdcard()) {
            File cacheDir = NetUtils.getApp().getExternalCacheDir();
            if (cacheDir == null) {
                result = new File(Environment.getExternalStorageDirectory(),
                        ".android/" + NetUtils.getApp().getPackageName() + "/" + dirName);
            } else {
                result = new File(cacheDir, dirName);
            }
        } else {
            result = new File(NetUtils.getApp().getFilesDir(), dirName);
        }
